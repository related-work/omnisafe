...
class DisQCritic(Critic):
    ...
    def __init__(
        ...
        self._output_dim = output_dim  # ✅必须有
        ...
        for idx in range(self._num_critics):
            if self._use_obs_encoder:
                ...
                net = build_mlp_network(
                    ...
                    [hidden_sizes[0] + self._act_dim] + hidden_sizes[1:] + [output_dim],
                    activation=activation,
                    weight_initialization_mode=weight_initialization_mode,
                )
                ...
                net = build_mlp_network(
                    ...
                    [self._obs_dim + self._act_dim, *hidden_sizes, output_dim],     
                    activation=activation,
                    weight_initialization_mode=weight_initialization_mode,
                )
    ...
    def forward(
        ...
        for critic in self.net_lst:
            if self._use_obs_encoder:
                ...
                out = critic[1](torch.cat([obs_encode, act], dim=-1))  # [B, output_dim]
            else:
                # res.append(torch.squeeze(critic(torch.cat([obs, act], dim=-1)), -1))
                out = critic(torch.cat([obs, act], dim=-1))            # [B, output_dim]
            # ✅ 兼容旧行为：output_dim==1 时保持输出 [B]
