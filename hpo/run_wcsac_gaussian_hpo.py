#!/usr/bin/env python3
"""Optuna search dedicated to Gaussian WCSAC."""
...
def suggest_params(trial: 'optuna.Trial') -> dict[str, Any]:
    """Sample only Gaussian WCSAC hyperparameters."""
...
def main() -> None:
    """Run Gaussian WCSAC HPO."""
    parser = argparse.ArgumentParser(description='Gaussian WCSAC HPO with Optuna')
    ...
    summary_path = os.path.join(common.OUTPUT_DIR, 'wcsac_gaussian_hpo_summary.yaml')
    with open(summary_path, 'w', encoding='utf-8') as file:
        yaml.safe_dump(results, file, allow_unicode=True, sort_keys=False)
    print(f'Gaussian WCSAC HPO 完成，结果保存到 {summary_path}')
